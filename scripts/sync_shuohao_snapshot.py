#!/usr/bin/env python3
"""Verify or update the pinned shuohao short-drama engine snapshot."""
...
UPSTREAM_DIR = ROOT / "vendor" / "shuohao" / "upstream"
ADAPTED_DIR = ROOT / "engine" / "shuohao-adapted"
RUNTIME_DIR = ROOT / "engine" / "shuohao-runtime"
MANIFEST_PATH = ROOT / "vendor" / "shuohao" / "snapshot-manifest.json"
...
def validate_source(source: Path) -> dict[str, str]:
    ...
    if not match or int(match.group(1)) < 18:
        raise RuntimeError("shuohao engine requires Node.js 18+")
    ...
    if "shuohao-skills" not in notice_text or "Licensed under the Apache License" not in notice_text:
    ...
    try:
        ...
        for staged in (upstream_stage, adapted_stage, runtime_stage, manifest_stage):
            if staged is not None and staged.exists():
                remove_path(staged)
        raise
    print(f"updated shuohao snapshot: {len(manifest['files'])} files")
