...
description: |
  ...
  出图风格可指定（--style），默认半写实，也可以出手绘赛璐璐动画风。
...
用户可以指定出图风格：**默认 `realistic`**（半写实厚涂），想要动画质感就用 `hand-painted-cel`（手绘赛璐璐式手绘赛璐璐）。
...
最容易搞反的是反向提示词：`realistic` 绝不能禁 `photorealistic`，`hand-painted-cel` 必须禁。`validate` 会拦这个。
