...
`checkup` 在质量门之外附**写稿手感提示（不设门）**：delivery 只写可执行策略不写情绪词、动作面不堆转折标记词、不出现编剧旁白。提示不影响通过判断，也不改退出码——但它们是决定戏好不好看的最后一层。
...
158 项断言，不调模型、不花额度。10 道质量门每一道都有击穿用例，写稿手感提示同样有击穿与不误报用例。改完脚本先跑这个。
