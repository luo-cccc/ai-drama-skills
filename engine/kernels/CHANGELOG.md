...
hand-painted-cel）但内容是环境版——真实感来自用旧的材质，不是皮肤毛孔。
...
- 新增顶层 `style` 字段，默认 `realistic`（半写实厚涂），另有 `hand-painted-cel`
  （手绘赛璐璐式手绘赛璐璐）
...
- **两个预设的反向提示词几乎是相反的**：`realistic` 绝不能禁
  `photorealistic`，`hand-painted-cel` 必须禁。`validate` 会拦这个搞反
- 表面处理同理：写实要毛孔、皮下散射、根根碎发、布料织纹；手绘赛璐璐明确
  要无毛孔、成簇发丝、平涂无织纹
- 光照也不同：写实分区打光（左栏方向光、右侧平光），手绘赛璐璐全图平光——
  平光本来就是那个风格的一部分
...
已用《渡口》端到端验证：日文 + 手绘赛璐璐跑通（界面日文、引文保持中文原文、
