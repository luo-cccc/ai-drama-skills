...
```
/novel-characters ./book.txt --style hand-painted-cel
```

| id | What it is |
| --- | --- |
| `realistic` | Semi-realistic painterly — skin with pores and texture, fabric with weave and wear. Default |
| `hand-painted-cel` | hand-painted-cel-like hand-painted cel — even ink linework, a single soft shadow tone, flat colour |

They combine: `--lang ja --style hand-painted-cel`.

```bash
node scripts/novel-characters.mjs styles          # list all presets
node scripts/novel-characters.mjs styles hand-painted-cel   # dump one in full
```
...
| **Style matches its negative prompt** | `realistic` must not ban `photorealistic`, `hand-painted-cel` must — get it backwards and the whole batch is wasted |
...
references/
  ...
  style-presets.md       image style presets (realistic / hand-painted-cel)
examples/
